...
const { ShardingManager, MessageFlags } = require('discord.js');
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (shardManager.shards.size <= 1) {
            return interaction.reply({ content: 'Cannot delete the shard. There must be more than 1 shard.', flags: MessageFlags.Ephemeral });
        }
        ...
        try {
            ...
            console.error(error);
            await interaction.reply({ content: 'Failed to delete the shard.', flags: MessageFlags.Ephemeral });
        }
    },
};
