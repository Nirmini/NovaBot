...
async function getOrCreateWebhook(guild) {
    ...
    // Check if we already have a webhook URL stored
    const webhookURL = await getGuildConfig(guild.id, 'featuremodules.actionlogs.webhookURL');
    if (webhookURL) return new WebhookClient({ url: webhookURL });

    // Create a new webhook if not found
    const webhook = await logChannel.createWebhook({
        name: "Nova",
        avatar: "https://i.imgur.com/G1x3K0f.png"
    });

    // Save the webhook URL in the correct config location
    await setGuildConfig(guild.id, 'featuremodules.actionlogs.webhookURL', webhook.url);
